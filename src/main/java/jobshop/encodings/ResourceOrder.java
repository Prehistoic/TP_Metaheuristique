...
public class ResourceOrder extends Encoding {
    ...
    public void addTask(int resource, int job, int task) {
        Task t = new Task(job,task);
        this.tasksByMachine[resource][this.nextFreeSlot[resource]++] = t;
    }

    public int getTaskIndex(int machine, Task t) {
        int index = -1;
        for(int i=0; i<instance.numJobs; i++) {
            if(tasksByMachine[machine][i].job == t.job && tasksByMachine[machine][i].task == t.task) {
                index = i;
            }
        }
        return index;
